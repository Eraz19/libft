...
/*   Updated: 2026/01/21 10:40:49 by adouieb          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void test_gnl_many_fds(void)
{
	...
	for (int i = 0; i < 10; i++)
	{
		if (fds[i] >= 0)
		{
			t_dstr result = get_next_line((t_u32)(fds[i]), &readers);
			if (result.s == NULL)
				success = 0;
			free_dstr(&result);
		}
